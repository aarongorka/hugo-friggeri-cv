...
A port of [friggeri-cv] to Hugo using datasources. The [Resumic] standard is used as the schema.

[Resumic]: https://github.com/resumic/schema
[friggeri-cv]: https://www.overleaf.com/latex/templates/friggeri-cv-template/hmnchbfmjgqh?nocdn=true
...
```toml
theme = "hugo-friggeri-cv"
```

Then, fill in a JSON (or YAML) file at `data/cv.json`.

## Schema

Here's an example [Resumic] schema.
...
Firefox does not render PDFs properly due to [this bug].

[this bug]: https://bugzilla.mozilla.org/show_bug.cgi?id=1089549
